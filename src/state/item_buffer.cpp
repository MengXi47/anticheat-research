...
namespace state::item_buffer {

namespace {

using FramePtr = std::shared_ptr<const std::vector<Item>>;

FramePtr g_latest = std::make_shared<const std::vector<Item>>();
...
void Publish(std::vector<Item> frame) {
  auto next = std::make_shared<const std::vector<Item>>(std::move(frame));
